...
def nab_score(y_true, y_predict, probationaryPercent=0.15, type_profile='standard'):
  sweeper = Sweeper(
    probationPercent=probationaryPercent,
    costMatrix=profile[type_profile]['CostMatrix']
  )

  result = []
  for algorithm in ['custom', 'null', 'random']:
    if algorithm == 'custom':
      anomalyScores = y_predict
    elif algorithm == 'null':
      anomalyScores = np.zeros(y_true.shape[0])
    elif algorithm == 'random':
      anomalyScores = np.random.uniform(0, 1, y_true.shape[0])

    timestamps = np.arange(len(y_true))

    t_start = timestamps[0]
    windows = []
    for ind in range(1, y_true.shape[0]):
      if y_true[ind] == 1 and y_true[ind - 1] == 0:
        t_start = timestamps[ind]
      if y_true[ind] == 1 and ind == y_true.shape[0] - 1:
        windows.append((t_start, timestamps[ind - 1]))
      if y_true[ind] == 0 and y_true[ind - 1] == 1:
        windows.append((t_start, timestamps[ind - 1]))
    ...
    scoresByThreshold = sweeper.calcScoreByThreshold(curAnomalyRows)
    scoresByThreshold = sorted(scoresByThreshold, key=lambda x: x.score, reverse=True)
    bestParams = scoresByThreshold[0]

    print("{} algorithm: optimizer found a max score of {} with anomaly threshold {}.".format(
      algorithm, bestParams.score, bestParams.threshold
    ))
    result.append((algorithm, bestParams.score, bestParams.threshold))
